# Alert_AutoFix
## 概要 このFlaskアプリケーションは，Alertmanagerから送信されるアラートをJSON形式で受け取り，Gemini APIを用いて自動的に対処スクリプトを生成，評価，再生成するWebhookエンドポイントです． 生成されたスクリプトは実行後に結果を解析し，有効でない場合には再度プロンプトを修正してスクリプトを再生成します．
## 環境 
